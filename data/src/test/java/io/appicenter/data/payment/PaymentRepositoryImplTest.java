...
import io.appicenter.data.payment.service.ServiceTestData;
...
class PaymentRepositoryImplTest {
    ...
    private PaymentRepositoryImpl underTest;
    private PaymentService paymentService;
    private final MockWebServer mockWebServer = new MockWebServer();
    private final TestObserver<List<PaymentMethod>> testSubscriber = new TestObserver<>();
    private final PaymentMethodModelAdapter adapter = new PaymentMethodModelAdapter();
    ...
    @BeforeEach
    void setUp() {
        paymentService = new PaymentServiceImpl(api);
        underTest = new PaymentRepositoryImpl(paymentService, adapter);
    ...
    class Http_OK {
        ...
        class PaymentMethodNotEmpty {
            ...
            public void shouldReturnList() throws IOException {
                ...
                PaymentMethod firstMethod = testSubscriber.values().get(0).get(0);
                assertThat(testSubscriber.values().get(0)).hasSize(17);
                assertThat(firstMethod.code).isEqualTo(ServiceTestData.CODE);
                assertThat(firstMethod.getClass()).isAssignableTo(PaymentMethod.class);
